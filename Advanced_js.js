...
console.log(" \n QUESTION 1 : Filter by users who post atleast once based on their timestamp \n");
// 1. Filter by users who post atleast once based on their timestamp
...
console.log(" \n QUESTION 2 : Filter out those with less than 10 likes \n");
...
const usersPostLikes = users.map(user => {
    ...
    console.log(`${user.name} has ${user.posts.length} posts after filtering`)
);



console.log(" \n QUESTION 3 : CALCULATING AVERAGE LIKES PER POST \n");
...
const usersAverageLikes = users.map(user => {
    ...
    console.log(`${user.name} has an average of ${user.averageLikes.toFixed(2)} likes per post`) // 2 decimal places.
);
...
console.log(" \n QUESTION 4 : Calculate total bill for items above 450 \n");
